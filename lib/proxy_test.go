...
func TestGet2(t *testing.T) {
	p := new(ProxyManager)
	psrv1 := new(proxyServer)
	psrv1.Addr = new(net.UDPAddr)
	psrv2 := new(proxyServer)
	psrv2.Addr = new(net.UDPAddr)
	p.init()
	p.proxies["1"] = psrv1
	p.proxies["2"] = psrv2

	get := p.get()
	if !reflect.DeepEqual(get, p.proxies) {
		t.Error("Error. Return wrong values")
	}
}
